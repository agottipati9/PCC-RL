...
// }
//
void load_trace_file(const std::string& file_name, struct) {
    std::ifstream in_file(file_name);
    std::string line;
    std::vector<float> cooked_time;
    std::vector<float> cooked_bw;
...
int main(int argc, char* argv[]) {
  ...
  for (unsigned int trace_idx = 0;
       trace_idx < all_cooked_time_bw.all_cooked_time.size(); trace_idx++) {
  ...
  //         restore_or_compute_download_time(all_download_time, i, j, k,
  //                                          quan_time, quan_bw, DT,
  //                                          video_sizes[k]);
  //       }
  //     }
  //   }
